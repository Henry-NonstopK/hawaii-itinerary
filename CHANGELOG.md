...
- 채팅 입장 화면을 이름만 입력하는 구조로 단순화하고 방을 나갈 때까지 이름 유지
- Supabase Realtime 이벤트 누락 시에도 새로고침 없이 갱신되도록 채팅 1.5초 동기화 fallback 추가
- 사진 탭을 Supabase Storage 기반으로 전환하고, 모바일 카메라 업로드 / 앨범 선택 / 썸네일 확대 / 삭제 흐름을 추가
- 사진 목록은 공개 storage bucket `hawaii-photos`에서 불러오고, 다른 기기에서도 새로고침으로 동기화되도록 갱신 주기 추가
- 사진 업로드 시 50MB 기준으로 자동 압축하도록 보강
